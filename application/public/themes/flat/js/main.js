...
var webApp = function() {
    ...
    var uiInit = function () {
        ...
        pageContent.css('min-height', $(window).height() -
            ($('header').outerHeight() + $('#pre-page-content').outerHeight() + parseInt(pageContent.css('padding-top'))*2 + $('footer').outerHeight()) + 'px');
            ...
            if (link.next('ul').length > 0) {
                if (link.parent().hasClass('active') !== true) {
                    if (link.hasClass('open')) {
                        link.removeClass('open').next().slideUp(150);
                    }
                    else {
                        $('#primary-nav li > a.open').removeClass('open').next().slideUp(150);

 ...
 * @version 2.13
 * 
 * https://github.com/lonnieezell/Ocular-Template-Library/wiki
 */
class Ocular_Template {
	 ...
	 * @return void
	 */
	protected function _mark($name='', $override=FALSE) 
	{
	 ...
	 * @return void
	 */
	protected function _check_view($name='', $use_default=FALSE) 
	{
		if (!empty($name))
		{
	 ...
	 * @return	boolean
	 */
	protected function _render_view($view_name='', $cache_me=false, $data=null)
	{
	...
	protected function _check_layout($name='', $use_default=FALSE) 
	{	
		if (!empty($name))
		{
	 ...
	 */
	protected function is_cached($type='view') 
	{ 
	...
	protected function write_cache($content=null) 
	{
...
class Template extends Ocular_Template {
	...
	/**
	*
	* override _render_view ，讓他可以在 OCU_use_mobile_themes false 時，直接讀取目前設定的 default agent theme
	*
	* @access	public
	...
	*/
	protected function _render_view($view_name='', $cache_me=false, $data=null)
	{
		...
		} else if (empty($content) && empty($this->active_theme))
		{
			...
			{
				// 小白改的
				$view_name = $this->default_theme . '/' . $view_name;
				$content = $this->ci->load->view($view_name, $data, true);
			}
		}
	...
	/**
	*
	* override render ，讓他可以在 OCU_use_mobile_themes false 時，直接讀取目前設定的 default agent theme
	*
	* @access	public
	* @param	param (type) : param description
	* @return 	return : return description
	...
	{	
		...
		{	
			...
			{	
				// We're not using themes, so default to the 'views' folder
				// 小白修改的 
				$layout = $this->default_theme . '/' . $layout;
				$output = $this->ci->load->view($layout, $this->data, true);
				if (empty($output))
				{
					// Show an error here, since we're overriding CI's loader.
	...
	*/
	...
	/**
	*
	* 顯示 layout folder 下的某個 block view
	*
	* @access	public
	* @param	param (type) : param description
	* @return 	return : return description
